...
$select4 = mysqli_query($conn, "SELECT * FROM `tblmovie` WHERE movie_id = '$id'") or die('query failed');
...
<body>
    ...
    <div class="form-container">
        <form action="" method="post">
            <a href="home.php" class="cta-button"> <i class="fas fa-home"></i></a>
            <h1> ORDER</h1><br>
            <p>Movie ID :
                <?php echo $id; ?>
            </p>
            ...
            <image src="css/images/Layout.png" id="seatimage">
                <div class="wrapper">
                    Payment Mode:
                    ...
                    <div class="wrapper">
                        <div class="card">
                            <input class="inputrad" type="radio" name="card" value="vip"
                                onclick="changeImage('css/images/VIP.png' , '<?php echo 'Rp'; echo number_format($fetchcon['vip_price'], 2); ?>'); "
                                id="radioinput">
                            <span class="check"></span>
                            <label class="labelrad">
                                <div class="title">VIP</div>
                                <?php echo 'Rp';  echo number_format($fetchcon['vip_price'], 2); ?>
                            </label>
                        </div>
                        <div class="card">
                            <input class="inputrad" type="radio" name="card" value="ub"
                                onclick="changeImage('css/images/UB.png','<?php echo 'Rp'; echo number_format($fetchcon['ub_price'], 2); ?>'); "
                                id="radioinput">
                            <span class="check"></span>
                            <label class="labelrad">
                                <div class="title">UPPER</div>
                                <?php echo 'Rp'; echo number_format($fetchcon['ub_price'], 2); ?>
                            </label>
                        </div>
                        <div class="card">
                            <input class="inputrad" type="radio" name="card" value="mb"
                                onclick="changeImage('css/images/MB.png','<?php echo 'Rp'; echo number_format($fetchcon['mb_price'], 2); ?>'); "
                                id="radioinput">
                            <span class="check"></span>
                            <label class="labelrad">
                                <div class="title">MIDDLE</div>
                                <?php echo 'Rp'; echo number_format($fetchcon['mb_price'], 2); ?>
                        ...
                        <div class="card">
                            <input class="inputrad" type="radio" name="card" value="lb"
                                onclick="changeImage('css/images/LB.png','<?php echo 'Rp'; echo number_format($fetchcon['lb_price'], 2); ?>'); "
                                id="radioinput">
                            <span class="check"></span>
                            <label class="labelrad">
                                <div class="title">LOWER </div>
                                <?php echo 'Rp'; echo number_format($fetchcon['lb_price'], 2); ?>

                            </label>
                        </div>
                        <div class="card" onclick="resetRadio()"
                            style="background-color: black; color: red; font-weight: bolder; font-size: 20px; height: 7vh; opacity: 0.7;">
                    ...
                    <a href="pickseatscredit.php" class="cta-button2">Choose a Seat</a>
                    ...
                    <p id="total">Total Amount: <span id="result">
                            <?php echo 'Rp'; echo number_format($totalPrice, 2); ?>
                        </span></p>
                        ...
                        if($enteredPin == $storedPin) {
                            ...
                            if(!is_null($selectedSeatIds) && !empty($selectedSeatIds)) {
                                ...
                                for($i = 0; $i < $count; $i++) {
                                    ...
                                    $query = "INSERT INTO `chosenseats` (movieid, seatid, seatnames, status) VALUES('$movieId', '$seatId', '$seatNames  ', 'Taken')";
                                ...
                                $insertuser = "INSERT INTO `tblbuyer` (buyer_id, buyer_name, buyer_chosenseats, payment_mode, buyer_phonenum, movie_name, movie_id, movie_date, tickets_qty, payment_date, transaction_no,payment_price, status)
                                    VALUES ('$user_id', '{$fetchuser['fullname']}', '$selectedSeats', 'Credit Card', '{$fetchuser['phonenum']}', '{$fetchcon['movie_name']}', '{$fetchcon['movie_id']}', '{$fetchcon['movie_date']}', '$selectedSeatsCount', NOW(), '$transactionNumber',$totalPrice, 'Paid')";
...
function redirectToAnotherPage(movieId, seats, quantity, seatscount, transactionNumber) {
    if (pinCorrect) {

        window.location.href = 'paymentinfocredit.php?movie_id=' + movieId + '&selected_seats=' + seats +
