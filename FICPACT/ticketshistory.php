<?php
    ...
    if(isset($_GET['delete'])) {
        ...
        foreach ($seatNames as $seatName) {
            ...
            $deleteOtherTableQuery = "DELETE FROM chosenseats WHERE seatnames = '$seatName'AND movieid='$conid'";
...
<body style="margin: 40px;">
    ...
    <div class="container">
        <div class="jumbotron">
            ...
            <p class="lead"> <b>By ARTCEPTRIS</b> </p>
        ...
        <form method="GET" action="ticketshistory.php" class="form-inline">
            <div class="form-group mr-3">
                <label for="searchInput" class="form-check-inline mr-3 text-white">Search:</label>
        ...
        <div class="table-responsive">
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                if ($row['status'] === 'Pending' || $row['status'] !== 'Paid') {
                    echo "<a class='btn btn-danger btn-sm text-white' onclick='cancelPayment(\"" . $row['transaction_no'] . "\", \"" . $row['movie_id'] . "\")'>Cancel Payment</a>";
    ...
    <script>
    function cancelPayment(transactionNo, movieId) {
        var confirmCancel = confirm('Are you sure you want to cancel the payment for Transaction No. ' + transactionNo +
            '?');
        if (confirmCancel) {
            window.location.href = 'ticketshistory.php?delete=' + transactionNo + '&movie_id=' + movieId;
        }
