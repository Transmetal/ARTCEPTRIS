...
$movieId = $_SESSION['movie_id'];
 ...
 $movie = mysqli_query($conn, "SELECT * FROM `tblmovie` WHERE movie_id = '$movieId'") or die('query failed');

if (mysqli_num_rows($movie) > 0) {
   $fetch = mysqli_fetch_assoc($movie);
 } else {
   $fetch = null;
}
...
<body>
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="card mt-3">
                    <div class="card-header">
                        <h2>Movie Name: <?php echo $fetch['movie_name']; ?><h2>
                                <h5>Movie Date: <?php echo $fetch['movie_date']; ?><h5>
            ...
            <div class="col-md-9 mt-3">
                <div class="card ">
                    ...
                    if(isset($_GET['seats']))
                    {
                        ...
                        {
                            $products = "SELECT s.*, cs.* FROM seats s  LEFT JOIN chosenseats cs ON s.seatid = cs.seatid AND cs.movieid ='$movieId' WHERE s.section = '$rowbrand'";
                        ...
                        <div class="col-md-4 mt-3">
                            <div class="border p-2">
                                ...
                                <input type="checkbox" name="selected_seats[]"
                                    ...
                                    onclick="limitCheckboxSelection(this, 2)" <?= $isDisabled ? 'disabled' : ''; ?> />
                        ...
                        <?php
                                ...
                                if ($selectedSeatsCount >= 2) {
                                    echo '<p class="checkbox-limit-message">You can only select up to 2 seats.</p>';
                                }
                            }
                        }
                    }
                    ...
                    {
                        $products = "SELECT s.*, cs.* FROM seats s LEFT JOIN chosenseats cs ON s.seatid = cs.seatid AND cs.movieid = '$movieId'";
                    if (isset($_GET['search']) && !empty($_GET['search'])) {
                     $search = $_GET['search'];
            ...
            <p class="p1">
                ...
                <span></span>
                <span></span>
                Anda hanya berkesempatan untuk memilih 2 seat dalam 1 user, hal ini disebabkan oleh keterbatasan kuota agar tidak terjadi kesalahan pada saat pengisian seat.
            </p>
        </div>
        </div>
    </div>
    ...
    document.querySelector('.cta-button').addEventListener('click', function() {
        ...
        this.href = '   cash.php?movie_id=' + <?php echo json_encode($movieId); ?> + '&encrypted_seats=' +
