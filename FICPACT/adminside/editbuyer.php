...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate Movie Name
    $movieName = validateInput($_POST["buyer_cname"]);
    if(empty($movieName)) {
        $errors[] = "Movie Name is required";
    }

    // Validate Movie Date
    $movieDate = validateInput($_POST["buyer_cdate"]);
    if(empty($movieDate)) {
        $errors[] = "Movie Date is required";
    }
    ...
    if(empty($errors)) {
        ...
        `movie_name`='$buyer_cname', `movie_date`='$buyer_cdate', 
        ...
        if($resultBuyer && ($buyer_status == 'Paid' || $buyer_status == 'Pending')) {
            ...
            $conid = isset($_GET['movie_id']) ? intval($_GET['movie_id']) : 0;
            ...
            foreach($seatNamesArray as $seatName) {
                ...
                $updateSeatsQuery = ($buyer_status == 'Paid') ?
                    "UPDATE `chosenseats` SET `status`='Taken' WHERE seatnames LIKE '%$trimmedSeatName%' AND movieid='$conid'" :
                    "UPDATE `chosenseats` SET `status`='Reserved' WHERE seatnames LIKE '%$trimmedSeatName%' AND movieid='$conid'";
...
<body>
    ...
    <div class="container">
        ...
        if($id > 0 && $movie_id > 0 && !empty($transaction_no)) {

            $query = "SELECT * FROM tblbuyer WHERE buyer_id = $id AND movie_id = $movie_id AND transaction_no = '$transaction_no' LIMIT 1";
            ...
            echo "Invalid ID, movie ID, or transaction number!";
        }
        ?>
                ...
                <div class="row">
                    ...
                    <div class="form-group mb-3 col">
                        ...
                        <input type="text" maxlength="50" class="form-control" name="buyer_cname"
                            value="<?php echo $row['movie_name'] ?>" required>
                    </div>
                    ...
                    <div class="row">
                        <div class=" mb-3 col">
                            ...
                            <input type="date" class="form-control" name="buyer_cdate"
                                value="<?php echo $row['movie_date'] ?>" required>
                        </div>
